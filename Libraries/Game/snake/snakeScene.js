...
export function resolveCenterSnakeSpawnAnchor(state, navWalkable, { segmentCount, excludeIndices = null }) {
    ...
    const spacing = resolveSnakeSegmentSpacing(snake.linkSlack, config.startRadius);
    return pickSnakeChainSpawnCellNearestTo(navWalkable.cells(), navWalkable, state, centerCell.col, centerCell.row, segmentCount, spacing, snake.growDirX, snake.growDirY, excludeIndices);
}
...
export function spawnSnakeChain(state, anchorCell, { excludeIndices = null, segmentCount, faction = null, teamIndex = 0, growDirX = null, growDirY = null, rng = Math.random } = {}) {
    ...
    applyAgentGameplay(snake.gameplay.leader, chain.head);
    for (let i = 1; i < chain.members.length; i++) applyAgentGameplay(snake.gameplay.body, chain.members[i]);
...
export async function spawnSnakeCavernScene(state) {
    ...
    withSeededRandom(state.mapSeed + config.cavern.mapSeedOffset, () => {
        ...
        const centerPack = spawnSnakeChain(state, centerAnchor, { excludeIndices, segmentCount: centerSegmentCount, teamIndex: 0 });
        ...
        for (let i = 1; i < specs.length; i++) {
            ...
            const pack = spawnSnakeChain(state, anchorCell, { excludeIndices, segmentCount: spec.segmentCount, teamIndex: i });
            snakes.push(pack);
