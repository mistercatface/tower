...
export function createRadioController({ registry, requestPause, requestResume, onShowLine, onHide }) {
    ...
    function showCurrentLine() {
        ...
        onShowLine({ participants: resolved.participants, line, lineIndex, lineCount: resolved.lines.length });
    }
    ...
    function startSession(conversationId, onComplete, state, { force = false } = {}) {
        ...
        if (!force && shouldSkipConversation(conversationId, state)) return false;
        session = { conversationId, resolved, lineIndex: 0, onComplete: onComplete ?? null };
        if (state) {
    ...
    return { startSession, fireTrigger, advance, end, isActive };
}
