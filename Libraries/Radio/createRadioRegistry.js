...
export function createRadioRegistry({ conversations, speakers }) {
    ...
    for (const [conversationId, conversation] of Object.entries(conversations)) {
        ...
        if (!trigger) continue;
        if (!conversationsByTrigger.has(trigger)) conversationsByTrigger.set(trigger, []);
    ...
    function getParticipantIds(conversation) {
        const ids = [];
        for (const line of conversation.lines) if (!ids.includes(line.speakerId)) ids.push(line.speakerId);
        return ids;
    }
    function resolveConversation(conversationId) {
        ...
        const lines = conversation.lines.map((line) => {
            ...
            if (!speaker) return null;
            return { speakerId: line.speakerId, speakerName: speaker.name, portrait: speaker.portrait, text: line.text };
        });
        if (lines.some((l) => l == null)) return null;
        return { id: conversationId, trigger: conversation.trigger, oncePerRun: conversation.oncePerRun ?? false, participants, lines };
    }
    return { getConversationIdsForTrigger, getConversation, getSpeaker, getParticipantIds, resolveConversation };
}
