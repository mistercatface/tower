...
class PixelParticle {
    ...
    renderScreen(ctx, viewport) {
        ...
        if (size <= 1) ctx.fillRect(px, py, 1, 1);
        else {
...
class SmokeParticle {
    ...
    renderScreen(ctx, viewport) {
        ...
        ctx.fillRect(Math.floor(screen.x - size / 2), Math.floor(screen.y - size / 2), Math.ceil(size), Math.ceil(size));
    }
}
export class CombatParticles {
    static spawnBlood(state, x, y, options = {}) {
        ...
        for (let i = 0; i < count; i++) {
            let angle;
            if (impactAngle != null) angle = impactAngle + Math.PI + (Math.random() - 0.5) * 1.0;
            else angle = Math.random() * Math.PI * 2;
            const speed = (120 + Math.random() * 100) * intensity;
            const vx = Math.cos(angle) * speed;
            const vy = Math.sin(angle) * speed;
            const life = 0.18 + Math.random() * 0.14;
            state.combatParticles.push(new PixelParticle(x + (Math.random() - 0.5) * 3, y + (Math.random() - 0.5) * 3, vx, vy, pickBloodColor(), Math.random() > 0.35 ? sizePx : 1, life, 4.5));
        }
    ...
    static spawnImpactSparks(state, x, y, options = {}) {
        ...
        for (let i = 0; i < count; i++) {
            let angle;
            if (kind === "ricochet" && impactAngle != null) angle = impactAngle + (Math.random() - 0.5) * 1.2;
            else if (impactAngle != null) angle = impactAngle + Math.PI + (Math.random() - 0.5) * 1.4;
            else angle = Math.random() * Math.PI * 2;
            const speed = 160 + Math.random() * 120;
            const vx = Math.cos(angle) * speed;
            const vy = Math.sin(angle) * speed;
            const life = 0.22 + Math.random() * 0.16;
            state.combatParticles.push(new PixelParticle(x + (Math.random() - 0.5) * 2, y + (Math.random() - 0.5) * 2, vx, vy, pickSparkColor(palette), Math.random() > 0.5 ? 2 : 1, life, 3));
        }
        trimParticles(state.combatParticles);
    }
    static spawnMuzzleFlash(state, x, y, angle, options = {}) {
        ...
        if (!isPellet || Math.random() > 0.5) {
            ...
            state.combatParticles.push(
                new SmokeParticle(x, y, Math.cos(angle) * drift + (Math.random() - 0.5) * 12, Math.sin(angle) * drift + (Math.random() - 0.5) * 12, 0.35 + Math.random() * 0.25),
            );
        }
        trimParticles(state.combatParticles);
    }
    static resolveImpactAngle(actor, event) {
        if (event?.projectile) return event.projectile.angle;
        if (event?.type === "blast" && event.explosion) return Math.atan2(actor.y - event.explosion.y, actor.x - event.explosion.x);
        return null;
    }
    ...
    static updateAll(state, dt) {
        ...
        for (let i = state.combatParticles.length - 1; i >= 0; i--) {
            ...
            p.update(dt);
            if (p.isDead) state.combatParticles.splice(i, 1);
        }
    }
