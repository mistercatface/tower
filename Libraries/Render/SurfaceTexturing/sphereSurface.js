...
export function sphereLocalVertex(radius, phi, theta) {
    ...
    return {
        lx: radius * sinPhi * Math.cos(theta),
        ly: radius * sinPhi * Math.sin(theta),
        z: radius * (1 + cosPhi),
    };
}
 ...
 */
export function tessellateSphereQuads({
    radius,
    rollQuat,
    phiMin,
    phiMax,
        ...
        for (let spi = 0; spi < subPhi; spi++) {
            ...
            for (let ti = 0; ti < thetaSegments; ti++) {
                for (let sti = 0; sti < subTheta; sti++) {
                    ...
                    cells.push({
                        depth: (m00.z + m01.z + m11.z + m10.z) * 0.25,
                        u0,
                        u1,
                        v0,
                        v1,
                        verts: [m00, m01, m11, m10],
                    });
                }
            }
        }
 ...
 * }} spec
 */
export function tessellateSphereCapQuads({
    radius,
    rollQuat,
    phiCenter,
    thetaCenter,
