...
export function resolvePoolBallFaceColor(face, poolBall) {
    ...
    if (poolBall.kind === "stripe" && vMid > 0.26 && vMid < 0.74) {
        return "#F5F5F5";
    }

    if (poolBall.kind === "cue") {
        return vMid < 0.2 || vMid > 0.8 ? shadeHex(base, 0.06) : base;
    }
...
export function poolBallFromNumber(number) {
    ...
    if (number === 0 || number > 15) {
        return { kind: "cue", color: "#F5F5F0" };
    }
    if (number <= 8) {
        return { kind: "solid", number, color };
    }
