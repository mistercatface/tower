...
export function seedRagdollBloodOnDeath(ragdoll, hitBone, rig) {
    ...
    for (const bone of burstBones) {
        ...
        for (let i = 0; i < bCfg.BURST_COUNT; i++) ragdoll.particles.push(createBloodParticle(burstPoint, bCfg, scale));
    }
}
...
export function updateBloodEffects(ragdoll, deltaSec, rig) {
    ...
    while (ragdoll.floorStains.length > bCfg.MAX_STAINS) ragdoll.floorStains.shift();
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
        const p = particles[i];
        if (p.onGround) p.life -= dt * (bCfg.GROUND_FADE ?? 0.1);
        else {
            p.life -= dt;
            ...
            if (p.y >= groundY) {
                ...
                p.vy = 0;
                p.onGround = true;
                if (Math.random() < 0.65) ragdoll.floorStains.push({ x: p.x, y: groundY, z: p.z, size: p.size * (2 + Math.random() * 1.5), color: bCfg.PALETTE.DRIED });
                p.size = (0.25 + 0.75 * (p.life / p.startLife)) * bCfg.SPLAT_SIZE;
            } else p.size = 0.25 + 0.75 * (p.life / p.startLife);
        }
...
function drawPixelStain(ctx, px, py, radius, color) {
    ...
    for (let y = -rInt; y <= rInt; y++) for (let x = -rInt; x <= rInt; x++) if (x * x + y * 2 * (y * 2) <= radius * radius && Math.random() > 0.25) ctx.fillRect(cx + x, cy + y, 1, 1);
    ctx.globalAlpha = 1;
}
function drawBloodDrop(ctx, proj, particle, config, bCfg) {
    ...
    for (let y = -Math.ceil(r); y <= Math.ceil(r); y++) for (let x = -Math.ceil(r); x <= Math.ceil(r); x++) if (x * x + y * y <= r * r) ctx.fillRect(cx + x, cy + y, 1, 1);
}
