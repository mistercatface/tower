...
function hairPalette(character) {
    return { base: character.hairColor, light: character.hairLight ?? character.hairColor, dark: character.hairDark ?? character.hairColor };
}
...
function drawMohawkHair(renderer, head, rig, palette) {
    ...
    const blob = r * 0.22;
    const yTop = -r * 0.62;
    for (let i = -2; i <= 2; i++) renderer.addSphere(offset(head, 0, yTop, i * r * 0.2), blob, palette);
}
const HAIR_STYLES = { buzzcut: drawBuzzcutHair, short: drawShortHair, mohawk: drawMohawkHair, barry: drawBarryHair, brock: drawBrockHair };
/**
