...
function prepareWallChunkPropTextures(state, eid) {
    const profileIdInt = entityWallProfileId[eid];
    if (!profileIdInt || !state?.worldSurfaces) return;
    const profileIdStr = getProfileStr(profileIdInt);
    const worldSurfaces = state.worldSurfaces;
    const wallHeightPx = entityWallHeightPx[eid];
    worldSurfaces.ensureWallChunkProfileTextures(state, profileIdStr, wallHeightPx);
    bindWallChunkTexturePipeline(worldSurfaces);
    const ready = worldSurfaces._wallChunkReady;
    const prop = entityRefs[eid];
    if (prop) {
        if (prop._wallChunkTextureReady !== ready) invalidatePropStaticKey(eid);
