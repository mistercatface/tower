...
function pairBroadphaseOverlapSlab(physIdA, physIdB) {
    const slab = kineticDynamicSlab;
...
function seedKineticBodyShapeFromBag(eid, bag) {
    const slab = kineticDynamicSlab;
...
function pairCircleCircleOverlapSlab(physIdA, physIdB) {
    const slab = kineticDynamicSlab;
...
export function compoundPartsMaxRadius(parts) {
    let maxR = 0;
    for (let i = 0; i < parts.length; i++) maxR = Math.max(maxR, parts[i].getBoundingRadius());
    return maxR;
}
export function applyCompoundFootprint(prop, flatVerts) {
    prop.drawOutline = new Float32Array(flatVerts);
    const partVerts = [];
    earClipConvexPartsInto(partVerts, flatVerts);
    releaseLivePolygon(prop);
    prop.collisionParts = partVerts.map((verts) => new PolygonShape(verts));
    prop.radius = compoundPartsMaxRadius(prop.collisionParts);
    prop.shape = prop.collisionParts[0];
}
export function markHitCompoundParts(parts, lx, ly) {
...
function circleCircleContact(xA, yA, rA, xB, yB, rB) {
    const dx = xB - xA;
    const dy = yB - yA;
    const distSq = dx * dx + dy * dy;
    const radii = rA + rB;
...
function isKinematicallyActiveSlab(physId) {
    const slab = kineticDynamicSlab;
    const vx = slab.vx[physId];
    const vy = slab.vy[physId];
...
function allowsKineticCollisionPairOrderSlab(physIdA, physIdB) {
...
function createWallHitBuffer(capacity = MAX_WALL_HITS) {
...
export function createKineticSession({ constraintsDirty = false, constraintsVersion = 0, topologyGeneration = 0, nextConstraintId = 1 } = {}) {
    ...
    return { kineticConstraintsDirty: constraintsDirty, kineticConstraintsVersion: constraintsVersion, kineticTopologyGeneration: topologyGeneration, nextConstraintId, substepPairPatchBodies: [], pairGatherFull: 0, pairGatherRefresh: 0, pairGatherPatch: 0, motionSubstepsRun: 0, motionSubstepsPlanned: 0 };
}
...
function contactWarmStartKeyFromPairKey(pairKey, featureA = 0, featureB = 0) {
...
function circleCircleContactSlab(physIdA, physIdB) {
    const slab = kineticDynamicSlab;
...
function bumpPairGatherStat(session, field) {
    if (field === "full") session.pairGatherFull++;
    else if (field === "refresh") session.pairGatherRefresh++;
    else session.pairGatherPatch++;
}
export function ensureKineticContactPairs(frame, world, outPairs) {
    ...
    if (patchEids?.length) {
        if (patchKineticPairsForEids(spatialFrame, outPairs, patchEids) > 0) bumpPairGatherStat(session, "patch");
        patchEids.length = 0;
    }
    return outPairs;
}
...
function classifyKineticPairTierSlab(physIdA, physIdB) {
    const slab = kineticDynamicSlab;
...
function pairPhysKey(physIdA, physIdB) {
...
export function patchKineticPairsForEids(spatialFrame, pairs, eids) {
    ...
    for (let i = 0; i < eids.length; i++) {
        const physIdA = eids[i];
        if (physIdA === undefined || physIdA === -1) continue;
...
function gatherKineticCandidatePairs(spatialFrame, pairs) {
    bakeSpatialNeighborCsr(spatialFrame);
    pairs.reset();
    const slab = kineticDynamicSlab;
    const neighborEids = slab.spatialNeighborEids;
...
function resolveActiveBodyWalls(spatialFrame, resolveWalls) {
    const slab = kineticDynamicSlab;
    for (let i = 0; i < slab.activePhysCount; i++) {
...
export function runCollisionPipeline(frame, world, resolveWalls, applyContactSideEffects, kineticIterations = collisionSettings.kineticIterations) {
    ...
    if (hasActiveBodies) {
        ...
        const patchBodies = world.kinetic.substepPairPatchBodies;
        for (let iter = 0; iter < kineticIterations; iter++) {
            ...
            if (!settled || iter === 0) resolveActiveBodyWalls(spatialFrame, resolveWalls);
...
function applyFloorBeltForces(world, spatialFrame, dtMs) {
    ...
    for (let i = 0; i < eidCount; i++) {
        ...
        if (turn === 1) {
            ...
            const v_lateral = entityVx[eid] * normalX + entityVy[eid] * normalY;
...
export function tickEntityFrames(frame, world, dt) {
    ...
    const eidCount = frame.kineticEidCount;
    ...
    for (let i = 0; i < eidCount; i++) {
        ...
        if (ageMs >= fadeOutMs) {
            const elapsedFade = ageMs - fadeOutMs;
...
export function runKineticPhysics(frame, world, dt, hooks) {
    ...
    for (let s = 0; s < steps; s++) {
        ...
        runCollisionPipeline(frame, world, hooks.resolveWalls, hooks.applyContactSideEffects);
        const maxSpeedSq = maxActiveKineticSpeedSq();
    ...
    session.motionSubstepsRun = substepsRun;
    session.motionSubstepsPlanned = steps;
...
function countMotionSubsteps(dtMs, { maxStepPx = 4, maxSubsteps = 8 } = {}) {
    const slab = kineticDynamicSlab;
    if (!slab.activePhysCount || dtMs <= 0 || maxStepPx <= 0) return 1;
    const dtSec = dtMs / 1000;
    let maxDisp = 0;
    for (let i = 0; i < slab.activePhysCount; i++) {
        const physId = slab.activePhysIds[i];
        if (slab.sleeping[physId]) continue;
        const disp = lengthXY(entityVx[physId], entityVy[physId]) * dtSec;
        if (disp > maxDisp) maxDisp = disp;
    }
    if (maxDisp <= 1e-6) return 1;
    return Math.min(maxSubsteps, Math.max(1, Math.ceil(maxDisp / maxStepPx)));
}
export function maxActiveKineticSpeedSq() {
    let max = 0;
    const slab = kineticDynamicSlab;
    for (let i = 0; i < slab.activePhysCount; i++) {
        const physId = slab.activePhysIds[i];
...
function physIdForBodyIdInList(eids, count, bodyId) {
    for (let i = 0; i < count; i++) {
        const physId = eids[i];
        if (kineticStaticSlab.entityId[physId] === bodyId) return physId;
    }
    return -1;
}
function bakeKineticIslandPlan(session, eids, count = eids.length) {
    ...
    for (let i = 0; i < constraints.count; i++) {
        const physIdA = physIdForBodyIdInList(eids, count, constraints.bodyAId[i]);
        const physIdB = physIdForBodyIdInList(eids, count, constraints.bodyBId[i]);
...
export function ensureKineticIslandPlan(session, eids, count = eids.length) {
    const version = getKineticConstraintsVersion(session);
...
function advanceKineticSleepIslands(frame, session) {
    const slab = kineticDynamicSlab;
    ...
    for (let i = 0; i < slab.activePhysCount; i++) {
        ...
        const vx = slab.vx[physId] || 0;
        const vy = slab.vy[physId] || 0;
        const speedSq = vx * vx + vy * vy;
        if (speedSq > sleepComponentMaxSpeedSq[root]) sleepComponentMaxSpeedSq[root] = speedSq;
        if (!canSleepKinetic(physId)) sleepComponentHasBlocker[root] = 1;
...
export function wakeKineticBody(eid) {
    ...
    if (sleeping[eid] === 0 && sleepFrames[eid] === 0) return;
    sleepFrames[eid] = 0;
    sleeping[eid] = 0;
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (peerPhysId === eid) continue;
        if ((entityFlags[peerPhysId] & ENTITY_FLAG_KINETIC) === 0) continue;
        sleepFrames[peerPhysId] = 0;
        sleeping[peerPhysId] = 0;
    }
}
function advanceKineticSleep(eid, eligible, requiredFrames = collisionSettings.kineticSleepFrames) {
    if ((entityFlags[eid] & ENTITY_FLAG_KINETIC) === 0) return;
    const sleeping = kineticDynamicSlab.sleeping;
    const sleepFrames = kineticDynamicSlab.sleepFrames;
    if (!eligible) {
...
export function applyGroundRollDrive(eid, dtSec) {
    ...
    if (kind === ROLL_DRIVE_BRAKE) return applyRollBrake(eid, dtSec, slab.rollDriveAccel[eid]);
    applyRollThrust(eid, dtSec, slab.rollDriveDirX[eid], slab.rollDriveDirY[eid], slab.rollDriveAccel[eid], slab.rollDriveMaxSpeed[eid]);
    return true;
}
function applyRollBrake(eid, dtSec, accel) {
    const vx = entityVx[eid];
    const vy = entityVy[eid];
    const speed = Math.hypot(vx, vy);
    ...
    if (speed <= decel) {
        ...
        entityVx[eid] = vx - (vx / speed) * decel;
        entityVy[eid] = vy - (vy / speed) * decel;
    }
    wakeKineticBody(eid);
    return true;
}
function applyRollThrust(eid, dtSec, dirX, dirY, accel, maxSpeed) {
    const len = Math.hypot(dirX, dirY);
    const dx = len > 0.001 ? dirX / len : 0;
    const dy = len > 0.001 ? dirY / len : 0;
    const desiredVx = dx * maxSpeed;
    const desiredVy = dy * maxSpeed;
    const steerX = desiredVx - entityVx[eid];
    const steerY = desiredVy - entityVy[eid];
    ...
    const speed = Math.hypot(entityVx[eid], entityVy[eid]);
    if (speed > maxSpeed) {
        entityVx[eid] = (entityVx[eid] / speed) * maxSpeed;
        entityVy[eid] = (entityVy[eid] / speed) * maxSpeed;
    }
    wakeKineticBody(eid);
}
