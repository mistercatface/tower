...
function registerAssetRecipes(asset, recipes) {
    ...
    if (!builder) throw new Error(`Unknown prop recipe "${asset.recipe}" for asset "${asset.id}"`);
    recipes[asset.id] = builder(asset.visuals);
    if (asset.id === "barrel") recipes.fire_barrel = PROP_RECIPE_BUILDERS.fuelBarrel(asset.visuals, { onFire: true });
}
