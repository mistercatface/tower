...
} from "../Spatial/transforms/longAxisBox3d.js";
...
export function usesLongAxisCollisionShape(prop) {
    if (prop.strategy?.rollAxis === "long" && !prop.strategy?.standTip) {
        return Boolean(prop.halfExtents || prop.strategy?.halfExtents);
    }
    if (isStandTipTilted(prop) || isStandTipFallen(prop)) {
        return true;
    }
    return false;
}
...
export function syncLongAxisCollisionShape(prop) {
    if (isStandTipProp(prop) && !isStandTipTilted(prop) && !isStandTipFallen(prop)) {
        ...
        if (!prop.shape || prop.shape.type !== "Circle") {
            prop.shape = new CircleShape(prop._baseRadius ?? prop.radius ?? 8);
        }
        return prop.shape;
    }
    ...
    if (prop._longAxisShapeKey === key && prop.shape?.type === "Polygon") {
        return prop.shape;
    }
