import { Handedness, defaultGunHandedness, dualServiceWeaponStart, equipmentLimits, randomLoadoutSettings } from "./equipmentConfig.js";
...
export function normalizeWeaponLoadout(gunIds) {
    ...
    if (twoHanded.length > 0) return [twoHanded[0]];
...
export function canEquipGun(loadout, gunId) {
    ...
    if (isTwoHandedGun(gunId)) return current.length === 0;
    if (current.some((id) => isTwoHandedGun(id))) return false;
...
function rollOneHandedLoadout(oneHandedPool) {
    ...
    if (oneHandedPool.length > 0 && Math.random() < randomLoadoutSettings.dualWieldOneHandedChance) guns.push(pickRandom(oneHandedPool));
    return normalizeWeaponLoadout(guns);
}
function rollDualServiceWeaponStart(pool) {
    const { gunId, chance } = dualServiceWeaponStart;
    if (!gunId || chance <= 0 || !pool.includes(gunId)) return null;
...
export function rollRandomLoadoutFromPool(pool) {
    ...
    if (dualService) return dualService;
    ...
    if (Math.random() < randomLoadoutSettings.twoHandedRollChance) return [pickRandom(twoHandedPool)];
...
export function getEquipmentSlotCount(loadout) {
    ...
    if (normalized.some((id) => isTwoHandedGun(id))) return 1;
...
export function getGunEquipAction(loadout, gunId) {
    ...
    if (equippedCount > 0 && !canEquipGun(normalized, gunId)) return "unequip";
    if (canEquipGun(normalized, gunId)) return "equip";
    if (equippedCount > 0) return "unequip";
    return "blocked";
}
export function equipGunToLoadout(loadout, gunId) {
    if (!canEquipGun(loadout, gunId)) return normalizeWeaponLoadout(loadout);
...
export function unequipGunFromLoadout(loadout, gunId) {
    ...
    return normalizeWeaponLoadout([...normalized.slice(0, index), ...normalized.slice(index + 1)]);
}
export function unequipSlot(loadout, slotIndex) {
    const normalized = normalizeWeaponLoadout(loadout);
    if (slotIndex < 0 || slotIndex >= normalized.length) return normalized;
    return normalizeWeaponLoadout([...normalized.slice(0, slotIndex), ...normalized.slice(slotIndex + 1)]);
}
export function toggleGunInLoadout(loadout, gunId) {
    const action = getGunEquipAction(loadout, gunId);
    if (action === "equip") return equipGunToLoadout(loadout, gunId);
    if (action === "unequip") return unequipGunFromLoadout(loadout, gunId);
