import { boundaryHopMouthOnSabPath } from "./boundaryNavHops.js";
...
export function boundaryHopIdxOnSabPath(worker, slot, pathLen, grid) {
    const mouth = boundaryHopMouthOnSabPath((i) => ({ col: worker.pathCol(slot, i), row: worker.pathRow(slot, i) }), pathLen, grid);
    if (!mouth) return null;
    for (let i = 0; i < pathLen; i++) if (worker.pathCol(slot, i) === mouth.col && worker.pathRow(slot, i) === mouth.row) return i;
    return null;
}
/**
 * @param {number} x
