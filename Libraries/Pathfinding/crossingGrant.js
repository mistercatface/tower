...
/**
 * @param {import("./navSession.js").NavSessionState & { boundaryHopIdx?: number | null, navPathActive?: boolean, crossingGrant?: CrossingGrant | null }} navState
 * @param {import("../Spatial/grid/WorldObstacleGrid.js").WorldObstacleGrid} grid
 */
export function refreshNavCrossingGrant(navState, grid) {
    navState.navPathActive = !!navState.path?.length;
    navState.crossingGrant = null;
    const hopIdx = navState.boundaryHopIdx;
    if (hopIdx == null || !navState.path?.length) return;
    if (navState.pathProgressIdx !== hopIdx) return;
    const wp = navState.path[hopIdx];
