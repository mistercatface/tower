...
function corridorCellIndex(c, r, layout) {
...
export function collectCorridorPathPointCells(p, prev, next, corridorWidth, interiorOnly, pathIndex, pathLength, layout) {
    ...
    if (alongH && alongV) {
        ...
        for (let oi = 0; oi < offsets.length; oi++) {
            ...
            const hk = corridorCellIndex(h.c, h.r, layout);
            const vk = corridorCellIndex(v.c, v.r, layout);
            if (!seen.has(hk)) {
                seen.add(hk);
                cells.push(h);
            }
...
export function corridorPathOccupiedCellIndices(path, corridorWidth, layout, options = {}) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        for (let ci = 0; ci < cells.length; ci++) indices.add(corridorCellIndex(cells[ci].c, cells[ci].r, layout));
    }
    return indices;
}
