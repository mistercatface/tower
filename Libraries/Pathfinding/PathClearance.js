...
function getWallsNearSegment(navGraph, ax, ay, bx, by, clearance) {
    ...
    return navGraph.getSegmentsInBounds(minX, minY, maxX, maxY);
}
...
function fixSegmentByTranslation(navGraph, a, b, clearance) {
    ...
    for (let iter = 0; iter < 6; iter++) {
        ...
        if (violation.t <= 0.5) {
            currentA = pushWaypointFromGeometry(navGraph, currentA.x + dx, currentA.y + dy, clearance);
        } else {
            currentB = pushWaypointFromGeometry(navGraph, currentB.x + dx, currentB.y + dy, clearance);
        }
        changed = true;
    }
...
export function adjustPathForClearance(path, navGraph, clearance, destination = null) {
    if (!path || path.length === 0 || !navGraph) {
        return path;
    }
    ...
    if (destination && adjusted.length > 0) {
        adjusted[adjusted.length - 1] = pushWaypointFromGeometry(navGraph, destination.x, destination.y, clearance);
    }
...
function isWorldSegmentClear(navGraph, ax, ay, bx, by, clearance) {
    ...
    const minY = Math.min(ay, by) - pad;
    const maxY = Math.max(ay, by) + pad;
    const walls = navGraph.getSegmentsInBounds(minX, minY, maxX, maxY);
    for (const wall of walls) {
        if (wall.isDead) continue;
...
export function orthogonalizePath(path, navGraph, clearance) {
    if (!path || path.length < 2 || !navGraph) {
        return path;
    }
    ...
    for (let i = 1; i < path.length; i++) {
        ...
        if (isDiagonal && !isWorldSegmentClear(navGraph, prev.x, prev.y, curr.x, curr.y, clearance)) {
            ...
            if (i >= 2) {
                ...
                const horizClear = isWorldSegmentClear(navGraph, prev.x, prev.y, viaHorizontalFirst.x, viaHorizontalFirst.y, clearance)
                    && isWorldSegmentClear(navGraph, viaHorizontalFirst.x, viaHorizontalFirst.y, curr.x, curr.y, clearance);
                const vertClear = isWorldSegmentClear(navGraph, prev.x, prev.y, viaVerticalFirst.x, viaVerticalFirst.y, clearance)
...
export function resolveRepositionTarget(navGraph, x, y, playerRadius) {
    ...
    if (
        clickCell.col < 0 || clickCell.col >= navGraph.cols ||
        clickCell.row < 0 || clickCell.row >= navGraph.rows ||
        navGraph.isBlocked(clickCell.col, clickCell.row)
    ) {
        return null;
    }

    const center = navGraph.gridToWorld(clickCell.col, clickCell.row);
    if (!canPlaceMoveTarget(navGraph, center.x, center.y, playerRadius)) {
        return null;
    }
