...
export function isWallCornerWaypoint(path, idx) {
    ...
    const to = path[idx + 1];
    if (!isAxisAlignedSegment(from, corner) || !isAxisAlignedSegment(corner, to)) return false;
...
function hasPassedWaypoint(x, y, from, to) {
    ...
    if (Math.abs(dx) >= Math.abs(dy)) return dx >= 0 ? x >= to.x - WAYPOINT_SLACK : x <= to.x + WAYPOINT_SLACK;
...
function getActiveWaypointIndex(x, y, path, arrivalDist, startIdx) {
    for (let i = startIdx; i < path.length; i++) {
        if (i > 0 && isWallCornerWaypoint(path, i)) {
            if (!hasPassedWaypoint(x, y, path[i - 1], path[i])) return i;
            continue;
        }
        const wp = path[i];
        const toWpX = wp.x - x;
...
export function computePathSteering(pose, path, targetX, targetY, settings = {}, navState = null) {
    ...
    const waypointArrival = Math.max(settings.pathWaypointArrival ?? 10, radius * 1.2);
    ...
    if (navState) navState.pathProgressIdx = Math.max(navState.pathProgressIdx ?? 0, activeIdx);
    const distToSteerTarget = Math.hypot(x - steerTarget.x, y - steerTarget.y);
    if (distToSteerTarget <= arrivalDistance) return { desiredX: 0, desiredY: 0, offPath: false };
    ...
    if (axisDir) {
        ...
        if (dirLen < 0.01) return { desiredX: 0, desiredY: 0, offPath: proj.dist > offPathDistance };
        dirX /= dirLen;
        dirY /= dirLen;
    }
    return { desiredX: dirX, desiredY: dirY, offPath: proj.dist > offPathDistance };
}
