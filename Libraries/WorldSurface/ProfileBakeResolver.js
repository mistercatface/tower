...
export function compileParamRef(path) {
    ...
    return {
        ...
        set(root, value) {
            ...
            if (curr != null) curr[parts[parts.length - 1]] = value;
        },
    };
}
...
function getScratchEntry(profileKey, baseProfile) {
    ...
    if (!entry || entry.base !== baseProfile) {
        entry = { base: baseProfile, scratch: cloneProfile(baseProfile), boundRefs: new Set() };
        scratchEntries.set(profileKey, entry);
    }
    return entry;
}
function resetBoundParams(entry) {
    for (const ref of entry.boundRefs) ref.set(entry.scratch, ref.get(entry.base));
}
function trackRefs(entry, bindings) {
    for (const binding of bindings) for (const ref of binding.refs) entry.boundRefs.add(ref);
}
...
export function createTimelineBinding(profile) {
    ...
    for (const stage of stages) if (stage.tracks) for (const track of stage.tracks) if (track?.targetPath) allTargetPaths.add(track.targetPath);
    const compiledTracks = Array.from(allTargetPaths).map((targetPath) => ({ targetPath, ref: compileParamRef(targetPath) }));
    if (compiledTracks.length === 0) return null;
    return {
        id: "timeline",
        ...
        apply(scratch, ctx) {
            ...
            for (const compiled of compiledTracks) {
                ...
                if (found) compiled.ref.set(scratch, value);
            }
        },
    };
}
...
export function buildBakeBindings(baseProfile, ctx) {
    const bindings = [];
    if (ctx.bindings?.length) bindings.push(...ctx.bindings);
    const timeline = createTimelineBinding(baseProfile);
    if (timeline) bindings.push(timeline);
    return bindings;
}
/**
...
export function resolveBakeProfile(baseProfile, profileKey, ctx = {}) {
    ...
    for (const binding of bindings) binding.apply(entry.scratch, ctx);
