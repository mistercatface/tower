...
export class ProgressiveFrameCache {
    ...
    setMeta(key, meta) {
        if (this.cache.has(key)) this._meta.set(key, meta);
    }
    getMeta(key) {
    ...
    mergeFrames(key, frameStart, newBitmaps) {
        ...
        for (let i = 0; i < newBitmaps.length; i++) merged[frameStart + i] = newBitmaps[i];
        this.set(key, merged);
    }
    delete(key) {
    ...
    getOrStart(key, meta) {
        ...
        if (meta !== undefined) this._meta.set(key, meta);
        return placeholder;
    }
    /**
    ...
    requestFill(key, fetchBatch, totalFrames) {
        if (!this._pendingFill.has(key)) this._pendingFill.set(key, { fetchBatch, totalFrames });
    }
    /**
    ...
    updateFills() {
        ...
        for (const [key, { fetchBatch, totalFrames }] of entries) {
            ...
            if (canvases.length >= totalFrames) continue;
            ...
            fetchBatch(batch).then((bitmaps) => {
                ...
                if (merged && merged.length < totalFrames) this.requestFill(key, fetchBatch, totalFrames);
            });
        }
    }
}
