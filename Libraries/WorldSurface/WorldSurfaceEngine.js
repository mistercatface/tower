...
export class WorldSurfaceEngine {
    ...
    drawGroundChunks(ctx, options) {
        ...
        for (const chunk of chunksToDraw) {
            ...
            if (zLevel > 0) {
                ctx.save();
                if (!clipChunkToRoofFootprints(ctx, wallSpatialIndex, chunk.origin.x, chunk.origin.y, chunkSizePx, zLevel, viewerX, viewerY, this.settings.cameraHeight, options.renderScene)) {
                    ctx.restore();
                    continue;
                }
                ...
                ctx.restore();
                drawRoofSegmentDamageOverlays(ctx, wallSpatialIndex, chunk.origin.x, chunk.origin.y, chunkSizePx, zLevel, viewerX, viewerY, this.settings.cameraHeight, options.renderScene);
