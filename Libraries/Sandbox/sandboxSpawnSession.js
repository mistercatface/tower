...
import { DEFAULT_SPLITTABLE_SPAWN_HEIGHT, DEFAULT_SPLITTABLE_SPAWN_WIDTH } from "./sandboxCapabilities.js";
...
export function createSandboxSpawnSession(state, { getSpawnPropId, pickSelection, notifyUi, placement }) {
    ...
    let spawnSplittableWidth = DEFAULT_SPLITTABLE_SPAWN_WIDTH;
    let spawnSplittableHeight = DEFAULT_SPLITTABLE_SPAWN_HEIGHT;
    const spawnCtx = () => ({
        ...
        spawnSplittableHalfExtents: { x: spawnSplittableWidth / 2, y: spawnSplittableHeight / 2 },
        pickSelection,
        notifyUi,
        placement,
    });
    ...
    return {
        ...
        getSpawnSplittableWidth: () => spawnSplittableWidth,
        setSpawnSplittableWidth: (width) => {
            spawnSplittableWidth = Math.max(6, Math.min(128, Math.round(width)));
            notifyUi();
        },
        getSpawnSplittableHeight: () => spawnSplittableHeight,
        setSpawnSplittableHeight: (height) => {
            spawnSplittableHeight = Math.max(6, Math.min(128, Math.round(height)));
            notifyUi();
        },
        spawnAt,
        spawnAtCameraOrigin() {
