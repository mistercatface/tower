...
export function createKinematicsPoses(config, rig) {
    const walk = {
        ...
        getTargets(cycle) {
            ...
            return { rightFoot: { x: fSwing * swingDist, y: rig.groundY - rLift }, leftFoot: { x: -fSwing * swingDist, y: rig.groundY - lLift } };
        },
        getModifiers(cycle) {
    ...
    const sneak = {
        ...
        getArmAngles(cycle) {
            ...
            return { rArm: 0.2 + sway, lArm: 0.2 - sway, rElbow: -0.5, lElbow: -0.5, rArmZ: 0.2, lArmZ: 0.2, rElbowZ: 0, lElbowZ: 0 };
        },
        getModifiers(cycle) {
            return { lift: -0.15 * rig.size, lean: 0.2 + Math.sin(cycle) * 0.02, bob: Math.cos(cycle * 2) * (rig.size * 0.01) };
        },
        getTargets(cycle) {
            const rawSwing = Math.sin(cycle);
    ...
    return { WALK: walk, SNEAK: sneak, ...buildStaticPoses(defaultStaticPoseDefs, rig) };
}
