...
import { PHYSICS_BONE_ALIASES, SEVER_LIMB_DEF, SEVER_TORSO_PEER } from "../skeleton/severing.js";
...
export function resolveSeverTarget(hitPart, ragdoll) {
    ...
    for (const segment of clean.split("_")) if (SEVER_MAP[segment]) return SEVER_MAP[segment];
    ...
    if (clean === "spineTop") {
        ...
        if (points.rShoulder && points.lShoulder) return Math.abs(rsZ) >= Math.abs(lsZ) ? "rArm" : "lArm";
        return Math.random() < 0.5 ? "rArm" : "lArm";
    }
    if (clean === "spineBot") {
        const rhZ = getRagdollPointZ(ragdoll, "rHip");
        const lhZ = getRagdollPointZ(ragdoll, "lHip");
        if (points.rHip && points.lHip) return Math.abs(rhZ) >= Math.abs(lhZ) ? "rLeg" : "lLeg";
...
export function severLimb(ragdoll, limbId, rig) {
    ...
    if (data.type === "simple") {
        ...
        for (const c of ragdoll.constraints) if (c.a === data.root) c.a = newPointId;
...
export function splitBone(ragdoll, boneStartName, t, rig) {
    ...
    if (basePart === "torso" || basePart === "spineTop" || basePart === "spineBot" || basePart === "head") return null;
    const { points, prevPoints, constraints } = ragdoll;
    let searchID = boneStartName;
    if (boneStartName.includes("_fr_")) searchID = boneStartName;
    else if (PHYSICS_BONE_ALIASES[boneStartName]) searchID = PHYSICS_BONE_ALIASES[boneStartName];
    let constraintIndex = -1;
    let foundConstraint = null;
    if (searchID === "spineTop" && !boneStartName.includes("_fr_")) constraintIndex = constraints.findIndex((c) => c.a === "spineTop" && c.b === "spineBot");
    ...
    } else constraintIndex = constraints.findIndex((c) => c.a === searchID);
    if (constraintIndex === -1) {
...
export function processRagdollGoreHit(ragdoll, forceX, forceY, forceZ, hitPart, damageVal, offsetT, rig) {
    ...
    if (cleanType === "head" || severTarget === "head") healthCategory = "head";
    else if (!severTarget && (cleanType.includes("spine") || cleanType === "torso")) healthCategory = "torso";
    ...
    if (ragdoll.partHealth[healthKey] === undefined) ragdoll.partHealth[healthKey] = maxHP;
    ...
    if (allowSever) action = "SEVER";
    else if (canFracture) action = "FRACTURE";
    if (action === "FRACTURE") {
    ...
    } else if (action === "SEVER") severLimb(ragdoll, severTarget, rig);
}
