...
export function massFromBody(body, defaultMass = getCollisionSettings().mass.pushableFallback) {
    if (body.mass !== undefined) return body.mass;
    if (body.radius !== undefined) return body.radius;
    return defaultMass;
}
/** @param {{ mass?: number, radius?: number }} body */
export function inverseMassFromBody(body, defaultMass = getCollisionSettings().mass.pushableFallback) {
