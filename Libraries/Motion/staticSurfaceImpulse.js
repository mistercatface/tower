...
export function applyStaticSurfaceImpulse(body, normalX, normalY, cx, cy, { restitution = 0, friction = 0.9 } = {}) {
    if (body.vx === undefined || body.vy === undefined) {
        return { approachDot: 0, applied: false };
    }
    ...
    if (approachDot >= 0) {
        return { approachDot, applied: false };
    }
    ...
    if (body.momentOfInertia) {
        body.angularVelocity = (body.angularVelocity || 0) + j * cross * invI;
    }
    ...
    if (body.momentOfInertia) {
        body.angularVelocity += jt * crossT * invI;
    }
