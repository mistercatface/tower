...
export class WallCollisionResolver {
    ...
    resolve(entity, spatialFrame) {
        ...
        const { collided, hits } = resolveBodyAgainstWallSegments(
            entity,
            entity.getShape(),
            candidateWalls,
            { restitution: wp?.restitution ?? 0.0, friction: wp?.friction ?? 0.9 },
        );

        if (this.onWallDamage) {
            for (let i = 0; i < hits.length; i++) {
                this.onWallDamage(entity, hits[i]);
            }
        }
