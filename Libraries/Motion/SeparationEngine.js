...
export class SeparationEngine {
    ...
    constructor(config) {
        ...
        this.neighborPad = resolved instanceof PairFilter ? (resolved.config.neighborPad ?? NEIGHBOR_QUERY_PAD) : (resolved.neighborPad ?? NEIGHBOR_QUERY_PAD);
    }
    /**
    ...
    compute(self, neighbors) {
        ...
        for (let i = 0; i < neighbors.length; i++) {
            ...
            accumulateSeparationFromPair(acc, self.x, self.y, self.radius, other.x, other.y, other.radius, this.neighborPad);
        }
