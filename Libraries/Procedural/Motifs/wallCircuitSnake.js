...
export const wallCircuitSnakeMotif = {
    metadata: {
        ...
        defaults: { type: "wallCircuitSnake", frequency: 0.022, threshold: 0.22, peak: 12, tint: [1.2, 0.35, 0.1], wiggleAmplitude: 24, octaves: 2, opacity: 0.7, blendMode: "add" },
        fields: [
    ...
    apply(sample, rgb, config) {
        if (!sample.isWall) return;
        const x = sample.evalX + (config.offset?.[0] ?? 0);
        const y = sample.evalY + (config.offset?.[1] ?? 0);
        const wallV = sample.wallV ?? 0;
        const wiggle = noise2D(x * (config.wiggleFrequency ?? 0.012), wallV * (config.wiggleScale ?? 10), 2) * (config.wiggleAmplitude ?? 30);
