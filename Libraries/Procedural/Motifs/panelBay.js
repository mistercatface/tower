...
export const panelBayMotif = {
    ...
    apply(sf, si, rf, ro, config, noise) {
        ...
        const rowH = 1 / rows;
        const colW = 1 / cols;
        const localV = (sf[SF_WALL_V] % rowH) / rowH;
        const localU = (sf[SF_WALL_U] % colW) / colW;
        ...
        if (distToEdge >= inset) {
            ...
            applyTint(rf, ro, -interior, [1, 1, config.interiorCool ?? 1.05]);
            return;
        }
