...
export function buildPortalSlotByKey(grid) {
    const index = new Map();
...
export function recomputePortalSlotIndex(grid) {
    grid.portalSlotByKey = buildPortalSlotByKey(grid);
}
...
export function findPortalEdgeByKey(grid, key) {
    ...
    if (slot) {
        ...
        if (isPortalEdge(edge)) return { col: slot.col, row: slot.row, side: slot.side, edge };
    }
    let found = null;
    forEachCellEdge(
        grid,
        (col, row, side, edge) => {
            if (canonicalEdgeCellKey(grid, col, row, side) === key) found = { col, row, side, edge };
        },
        { canonicalOnly: true, filter: isPortalEdge },
    );
    return found;
