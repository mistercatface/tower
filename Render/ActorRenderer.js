...
export class ActorRenderer {
    ...
    renderCombatHudClassic(ctx, renderer) {
        ...
        for (const turret of this.actor.getTurrets()) {
            turret.renderHudTriangle(ctx, renderer, this.actor);
        }
    }
    ...
    renderBars(ctx, cache, yOffset) {
        ...
        if (this.actor.health < this.actor.maxHealth && this.actor.healthBar) {
            secondaryOffset += this.actor.healthBar.height + 4;
        }

        const stunRatio = this.actor.getStunBarProgress();
        if (stunRatio != null && this.actor.stunBar) {
            this.actor.stunBar.render(ctx, this.actor.x, this.actor.y - secondaryOffset, stunRatio, cache);
        } else {
            const reloadRatio = this.actor.getReloadBarProgress();
            if (reloadRatio != null && this.actor.reloadBar) {
                this.actor.reloadBar.render(ctx, this.actor.x, this.actor.y - secondaryOffset, reloadRatio, cache);
            }
        }
    }
}
