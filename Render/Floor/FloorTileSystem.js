...
import {
    ...
    getPlayerAnchorBinding,
    profileHasTranslateTimeline,
} from "./floorTextureProfile.js";
import { drawBakedTexture } from "./floorTextureResolution.js";
import { PlayerTranslateTether } from "./PlayerTranslateTether.js";
...
export class FloorTileSystem {
    constructor() {
        ...
        this.translateTether = new PlayerTranslateTether();
        this._lastTetherUpdateMs = 0;
    }

    clear() {
        this.cache.clear();
        this._chunkBakeGeneration.clear();
        this.translateTether.reset();
        this._lastTetherUpdateMs = 0;
    }
    ...
    updateTranslateTether(state, profile) {
        const binding = getPlayerAnchorBinding(profile);
        if (!binding || !state.player || profileHasTranslateTimeline(profile)) {
            this.translateTether.reset();
            this._lastTetherUpdateMs = 0;
            return;
        }
        ...
        const dtMs = this._lastTetherUpdateMs > 0 ? now - this._lastTetherUpdateMs : 16;
        this._lastTetherUpdateMs = now;

        this.translateTether.setTarget(state.player.x, state.player.y);
        this.translateTether.update(dtMs);
    }

    getTetherBakeOrigin() {
        return this.translateTether.getBakeOrigin();
    }

    getChunkCanvas(chunkCol, chunkRow, state, priority = Infinity) {
        const profile = getFloorProceduralProfile(getFloorTextureProfileId(state));
        const tetherOrigin = this.getTetherBakeOrigin();
        const payload = buildFloorChunkBakePayload(state, chunkCol, chunkRow, tetherOrigin);
        const key = floorChunkCacheKey(chunkCol, chunkRow, payload.profileId, tetherOrigin);
    ...
    draw(ctx, state, viewport) {
        ...
        this.updateTranslateTether(state, profile);
