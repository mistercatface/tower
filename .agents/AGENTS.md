...
- **Prohibit Internal State Assertions**: Do not assert on private FSM transition reason strings, internal variable structures, or unexported state names in unit/integration tests.
    - **Reason**: Internal refactoring should not break tests as long as outward behavior remains identical. Assert strictly on public API methods, velocities, positions, or entity removals.
