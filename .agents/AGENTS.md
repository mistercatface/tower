...
- Grab/launch drag: closed-over scalars only (`grabPropId` + target/offset/anchor; launch aim floats). No `createDragLaunchAim` bag, no grab `propRuns`/`targetWorld`, no `createDragLaunchBehaviors` array wrapper — `createDragLaunchBehavior` returns one behavior.
