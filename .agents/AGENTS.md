...
- Modes (`SHAPE_TYPE_*`, `DRAW_KIND_*`, `ENTITY_KIND_*`, `ENTITY_FLAG_*`, …) live in `Core/engineEnums.js`. Slabs and buffer layout offsets (`VIEW_TIER_*`) live in `Core/engineMemory.js`. Eid allocate/bind/release lives in `Core/entitySlots.js`. Do not put semantic modes in `engineMemory`. Editor boot lives under `Apps/Editor/`, not a Core globals module.
...
- Overlay/nav hot path: `OVERLAY_DASH_PAIR`/`EMPTY` for `setLineDash` (no `[a,b]` literals); HPA/flow use `physicsSettings.groundNavRoll` + bare `physicsSettings.groundNavHpa.stopRadius` (no `getKineticRollConfig` / `_cachedRollBaseConfig`); flow path visit via Uint8 epoch bitset; selection overlay reuses module Set + props array.
...
- Ground nav: `createDirectGroundNavBehavior` / `createFlowGroundNavBehavior` / `createHpaGroundNavBehavior` own runs via `createGroundNavRunSlab` SoA in `engineMemory` (typed columns allocated at create; `sessions[]` for HPA). Run identity is **eid** (`eidToSlot` / `activeRunEids` / `slab.eid[slot]`) — no `propIdToSlot` / `activeRunIds` game-id dialect. Flags `GROUND_NAV_RUN_*` and identity `SANDBOX_BEHAVIOR_*` ints live in `engineEnums` — import those directly (no `*_BEHAVIOR_ID` aliases, no `rollToCursor*` / `grabDrag` strings, no per-prop run bags, no flow `topologyKey` string compare). Hot APIs take eid: `setMoveTarget(eid, x, y)` / `writeMoveTargetWorldInto(buf, o, eid)` / `clearMoveTarget(eid)`. Steer loops use `getRef(eid)` only — never `getLive` on the active-run path. `driveGroundNav(eid, …)` writes `N_OUT_STEER`. HPA path settings scratch writes only steering fields (no `Object.assign` of full nav.settings).
- Nav pose: `HpaNavSession.update/replan` and `PathReplanManager.trackStuck/evaluate*` take **eid** and read `entityX`/`entityY`/`entityR` — no WorldProp bags on that path.
- Kinetic frame membership: `KineticSpatialFrame.kineticEids` + `kineticEidCount` (all kinetic); awake list is `kineticDynamicSlab.activePhysIds` / `activeSlot` only — no `_kineticBodies` / `_activeKineticBodies` WorldProp arrays, no `prop._activeSlot`.
- Roll apply: `applyGroundRollDrive(eid)` writes `entityVx`/`entityVy`/`entityW` — never `entityRefs` on that path.
...
- Prop blit/bake: `drawCachedPropSprite` / `getOrBakePropSprite` / `getPropStaticKey` / `getPropStageBakeState` take **eid**; recipe via `entityRenderKeyId[eid]` → `propCatalogByRenderKeyId`; blit `entityX`/`entityY`. Stage fill / wall-chunk texture prep may `entityRefs[eid]` once for shape/strategy until later campaigns. `draw3DBuildings` / `_drawProp` loop edge: no `entityRefs`. Visibility cull uses `ENTITY_FLAG_RENDER_3D`; radial-sphere flat gate uses `ENTITY_FLAG_CIRCLE_SHAPE` (both stamped at bind) — no `getRef` + `strategy.renderMode` / bag shape chase at cull or `_drawProp`.
- Wall resolve on kinetic tick: `getWallCandidates(eid)` only (slab XY + `slabCollisionSpan`); `shouldResolveKineticBodyAgainstWalls(eid)`, `WallCollisionResolver.resolve(eid)`, part CSR SAT — no bag shape/`entityRefs` / `_wallCandidatesNearWorld` twin.
- Wall damage: `resolveKineticWallDamage(state, eid, frame)` uses `state.wallResolver` — no 4th resolver arg.
- Prop integrate on kinetic tick: `integratePropMotion(eid)` / `applyVelocityDamping(eid)` inline in `runKineticPhysics` substeps — no `entityRefs` / `updatePropSubstep` hook.
- Kinetic activity on tick: `isKinematicallyActiveSlab(eid)` / `allowsKineticCollisionPairSlab` only — no bag `isKinematicallyActive` / `needsWallCollision` / `shouldResolveKineticPair`.
