import { createRoguelikeWorldGenPort, ROGUELIKE_MAP_TOPOLOGY } from "../../Libraries/WorldGen/presets/roguelikeMap.js";
const labWorldGen = createRoguelikeWorldGenPort();
...
function calculatePath() {
    ...
    try {
        const result = currentWorld.hierarchicalNavigator.computePath(playerPos.x, playerPos.y, targetPos.x, targetPos.y);
...
function generateMap() {
    ...
    if (bounds && bounds.minX !== undefined) currentViewport.snapTo((bounds.minX + bounds.maxX) / 2, (bounds.minY + bounds.maxY) / 2);
...
function redrawCanvas() {
    ...
    renderMapLabView(canvas.getContext("2d"), canvas.width, canvas.height, currentWorld, currentViewport, readControls(), selectedNodeId, playerPos, targetPos, currentPath, currentAbstractPath);
...
function buildSettingsPanel() {
    ...
    addSlider("Node World Scale", 1, 20, 0.5, labWorldGen, "nodeWorldCoordScale");
...
function bootstrap() {
    ...
    buildSettingsPanel();
    const checkboxes = ["showNodesInput", "showRoomZonesInput", "showWallsInput", "showGridBoundsInput", "showPathDebugInput", "showPathTestInput"];
    ...
    canvas.addEventListener("pointerdown", (e) => {
        ...
        if (clickAction === "selectNode") {
            ...
            if (target) {
                ...
                calculatePath();
                redrawCanvas();
            } else updatePathStatus("Cannot reposition player: cell is blocked or has insufficient wall clearance.", true);
