...
export class Enemy extends Actor {
        ...
        for (const m of projectiles) {
            ...
            if (dist < 100 && !m.isDead) {
                ...
                if (Math.abs(angleDiff) < 0.5)
                    ...
                    } else this.dodgeTimerId = scheduler.schedule(500);
            }
        }
        return false;
    }
    isValidDodgeTarget(x, y, flowFieldGrid) {
        ...
        if (col >= 0 && col < flowFieldGrid.cols && row >= 0 && row < flowFieldGrid.rows) return flowFieldGrid.grid[row * flowFieldGrid.cols + col] === 0;
        return false;
    }
    canReposition(state) {
        return false;
