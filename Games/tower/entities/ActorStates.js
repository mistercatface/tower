...
import { enemyDefaults } from "../config/towerConfig.js";
...
function analyzeStrafePath(enemy, tangentX, tangentY, dir, walls, target, state) {
    ...
    for (let step = 1; step <= maxSteps; step++) {
        ...
        if (!hasLOS && coverDist === -1) coverDist = dist;
        if (hasLOS && openDist === -1) openDist = dist;
    }
...
export class EnemyNavigatingState {
    ...
    update(enemy, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state) {
        ...
        if (enemy.attackType === "charge") return enemy.changeStateAndUpdate("charging_prepare", null, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state);
...
export class EnemyEngagedState {
    ...
    update(enemy, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state) {
        ...
        if (dist > target.radius + enemy.weapon.range + 15) return enemy.changeStateAndUpdate("navigating", null, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state);
        ...
        if (shouldStrafe) {
            if (!hasLOS) return enemy.changeStateAndUpdate("navigating", null, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state);
            if (stateData.strafeDir === undefined) stateData.strafeDir = Math.random() < 0.5 ? 1 : -1;
            if (stateData.strafeTimerId === undefined || stateData.strafeTimerId === null) stateData.strafeTimerId = scheduler.schedule(8000 + Math.random() * 8000);
            ...
            if (scheduler.getTimeRemaining(stateData.strafeTimerId) <= 0) {
                const oppositePath = analyzeStrafePath(enemy, tangentX, tangentY, -stateData.strafeDir, walls, target, state);
                if (oppositePath.walkableDist > 50) stateData.strafeDir *= -1;
                stateData.strafeTimerId = scheduler.schedule(8000 + Math.random() * 8000);
            }
            const preferredDist = target.radius + enemy.weapon.range * 0.8;
            let radialFactor = 0;
            const distDiff = dist - preferredDist;
            if (Math.abs(distDiff) > 5) radialFactor = Math.max(-0.3, Math.min(0.3, distDiff * 0.01));
            ...
            if (stateData.linearStrafeState === undefined) stateData.linearStrafeState = "idle";
            if (stateData.linearStrafeTimerId === undefined || stateData.linearStrafeTimerId === null) stateData.linearStrafeTimerId = scheduler.schedule(200 + Math.random() * 500);
            if (scheduler.getTimeRemaining(stateData.linearStrafeTimerId) <= 0) {
                ...
                if (hasLOS) {
                    ...
                    if ((hasLeftCover || hasRightCover) && Math.random() < 0.7) {
                        ...
                        if (hasLeftCover && hasRightCover) stateData.strafeDir = leftPath.coverDist < rightPath.coverDist ? 1 : -1;
                        else stateData.strafeDir = hasLeftCover ? 1 : -1;
                    ...
                    if (hasLeftOpen || hasRightOpen) {
                        stateData.linearStrafeState = "strafing";
                        if (hasLeftOpen && hasRightOpen) stateData.strafeDir = leftPath.openDist < rightPath.openDist ? 1 : -1;
                        else stateData.strafeDir = hasLeftOpen ? 1 : -1;
                        const targetDist = stateData.strafeDir === 1 ? leftPath.openDist : rightPath.openDist;
                        stateData.linearStrafeTimerId = scheduler.schedule((targetDist / enemy.speed) * 1000 + 300);
                    } else return enemy.changeStateAndUpdate("navigating", null, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state);
                }
            }
...
export class EnemyChargePrepareState {
    update(enemy, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state) {
        if (enemy.chargeCooldown > 0) enemy.chargeCooldown -= dt;
        const distToTarget = Math.hypot(enemy.x - target.x, enemy.y - target.y);
        ...
        if (distToTarget > stagingDist + 25) enemy.calculateSteering(target, state);
...
export class EnemyChargeWindupState {
    ...
    update(enemy, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state) {
        ...
        const stateData = enemy.stateData;
        stateData.timer -= dt;
        if (stateData.timer <= 0) return enemy.changeStateAndUpdate("charging_dash", { timer: 1200 }, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state);
        return false;
    }
}
...
export class EnemyStunnedState {
    onEnter(enemy) {
        ...
        if (enemy.stateData.stunDurationMs == null) enemy.stateData.stunDurationMs = enemy.stateData.timer;
        if (!enemy.stateData.returnState) enemy.stateData.returnState = enemy.attackType === "charge" ? "charging_prepare" : "navigating";
    }
    ...
    update(enemy, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state) {
        ...
        if (enemy.stateData.timer <= 0) {
            if (enemy.attackType === "charge") enemy.chargeCooldown = 1500;
            enemy.vx = 0;
            enemy.vy = 0;
...
export class EnemyKnockedBackState {
    ...
    onEnter(enemy) {
        ...
        if (!data.returnState) data.returnState = enemy.attackType === "charge" ? "charging_prepare" : "navigating";
        data.phase = "push";
    ...
    resolveTurretAimAngle(enemy) {
        ...
        if (data.phase === "recovery") {
            const velLen = Math.hypot(enemy.vx, enemy.vy);
            if (velLen > 1) return Math.atan2(enemy.vy, enemy.vx);
        }
        return data.angle;
    }
    ...
    getStunBarProgress(enemy) {
        ...
        if (data.phase === "push") remaining += Math.max(0, data.stunMs - data.pushMs);
    ...
    update(enemy, dt, target, flowFieldGrid, walls, missiles, spatialFrame, scheduler, state) {
        ...
        if (data.timer <= 0) this.finish(enemy);
        return false;
    }
}
