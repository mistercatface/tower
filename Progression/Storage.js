...
function serializeProgress(state) {
    ...
    return {
        ...
        claimedPerkMilestones: Array.isArray(state.claimedPerkMilestones)
            ? state.claimedPerkMilestones.map((m) => asNonNegativeInt(m, 0))
            : [],
...
function applyProgress(state, upgrades, payload) {
    ...
    state.claimedPerkMilestones = Array.isArray(payload.claimedPerkMilestones)
        ? payload.claimedPerkMilestones.map((m) => asNonNegativeInt(m, 0))
        : [];
    state.discoveredAbilities = new Set(
        Array.isArray(payload.discoveredAbilities)
            ? payload.discoveredAbilities.filter((id) => typeof id === "string")
            : [],
    );
}
