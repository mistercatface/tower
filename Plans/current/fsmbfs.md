...
`gridReachabilityBfs.js` unchanged for flow cold path. ~~`FlowFieldWindow.checkReachability`~~ **deleted** (Pass 5).
...
All gates pass (zero hits in `*.js` except Plans markdown).

**Deleted:** `FlowFieldWindow.checkReachability`, `FlowFieldGrid.checkReachability` — dead code; decision reach is `navReachHorizon.js`. `gridReachabilityBfs.js` unchanged (flow cold path elsewhere if needed).

Docs synced: [`../AI.md`](../AI.md), [`normalization.md`](normalization.md), [`passthrough.md`](passthrough.md), [`../pathfinding.md`](../pathfinding.md).
...
- [x] One reach dialect: path steps everywhere in scoring/threat/cohesion
- [x] Perception + memory: **targets only**
- [x] `syncNavReachHorizon` / `navReachStepsTo` — no returned horizon objects
- [x] Reach computed **once** per agent at intent adapter — not threaded through enrich
- [x] No `Libraries/AI/decision/` or resolver getters
- [x] `deriveAllyState` reads `facts.reachSteps.ally`, not `known.allyDist`
- [x] No `*Dist` grep hits in `Libraries/`
- [x] Tests use `gridToWorld` + real sync — not mock `{ stepsTo }`
- [x] Net line count negative
- [x] New species: sync + `navReachStepsTo` — do not invent new dist fields
