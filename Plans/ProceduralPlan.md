...
## Pass 2 — `SeededNoise2D`

Replace module-global `activeNoiseMemo`, `currentNoiseSeed`, and free-function `noise2D` imports with a **`SeededNoise2D` class**: seed, `permCaches` pointer, per-pixel memo, `sample2D(x, y, octaves)`, `beginPixel()` / `endPixel()`, `static fromDerived(rootSeed, salt)`. `BakeSession` owns `memoryPool + noiseEvaluator`. Wire through `composeSurfaceImage`, `DomainWarp`, and motifs via `sampleScratch`. Exit: bit-identical bakes, no global memo side effects, derived sub-seeds ready for Tier 10.
...
Libraries/Procedural/Noise/Perlin2D.js      — permCaches, memo, noise profile gate
