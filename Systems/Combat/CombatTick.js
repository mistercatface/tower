...
import { MAP_TRAVEL_SPEED, completeMapTravel } from "./mapTravel.js";
...
/**
 * Ordered combat/map-travel simulation pass for CombatState.
 *
...
export function runCombatTick(ctx, dt) {
    ...
    if (!isTraveling && !abilityState.isDiving && ctx.state.player.applyQueuedTarget(ctx.state)) ctx.state.navigation.rebuildPlayerFlowField(ctx.state.player.targetX, ctx.state.player.targetY);
    ...
    if (!isTraveling) updateStartNodeIntro(ctx.state);
