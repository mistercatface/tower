...
export const playerLocomotionPhase = {
    run(ctx, dt, runtime) {
        ...
        if (!abilityState.isDiving && state.player.applyQueuedTarget(state)) {
            state.navigation.rebuildPlayerFlowField(state.player.targetX, state.player.targetY);
        }
        state.updateAllCombatants(dt, runtime.spatialFrame, {
            externalSpeedMod: abilityState.externalSpeedMod,
            upgrades,
            combatEvents: runtime.events,
        });
    },
};
...
export const inspectorPartyPhase = {
    run(ctx, dt, runtime) {
        ...
        for (const actor of state.getPlayerActors()) {
            actor.updateCombat(dt, state, runtime.spatialFrame, partyOpts);
        }
    },
};
