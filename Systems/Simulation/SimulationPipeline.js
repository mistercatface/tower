...
export function createSimulationPort(phases, options = {}) {
    ...
    const inspectorPipeline = options.inspectorPhases
        ? new SimulationPipeline(options.inspectorPhases)
        : null;

    return {
        runTick: (ctx, dt) => tickPipeline.runTick(ctx, dt),
        runInspectorTick: inspectorPipeline
            ? (ctx, dt) => inspectorPipeline.runTick(ctx, dt)
            : undefined,
