...
import { TILE_BAKE_TIER_REGISTRATION, TILE_BAKE_TIER_STATIC, TileBakeScheduler } from "../Libraries/WorldSurface/worldSurface.js";
...
describe("TileBakeScheduler", () => {
    it("coalesces duplicate ground-chunk enqueues to one worker post", () => {
        ...
        const first = scheduler.enqueue("bakeGroundChunk", payload, TILE_BAKE_TIER_STATIC);
        const second = scheduler.enqueue("bakeGroundChunk", payload, TILE_BAKE_TIER_STATIC);
    ...
    it("does not dedupe registration broadcast jobs", () => {
        ...
        const first = scheduler.enqueue("registerRuntimeProfile", payload, TILE_BAKE_TIER_REGISTRATION);
        const second = scheduler.enqueue("registerRuntimeProfile", payload, TILE_BAKE_TIER_REGISTRATION);
        const third = scheduler.enqueue("registerRuntimeProfile", payload, TILE_BAKE_TIER_REGISTRATION);
    ...
    it("drops obsolete revision jobs without posting to workers", async () => {
        ...
        scheduler.enqueue("bakeGroundChunk", chunkPayload({ chunkKey: packChunkKey(5, 0) }), TILE_BAKE_TIER_STATIC);
        const queued = scheduler.enqueue("bakeGroundChunk", chunkPayload({ chunkKey: packChunkKey(0, 0) }), TILE_BAKE_TIER_STATIC);
        revision = 2;
    ...
    it("dispatches nearer focus jobs first after camera moves", async () => {
        ...
        const far = scheduler.enqueue("bakeGroundChunk", chunkPayload({ centerX: 0, centerY: 0 }), TILE_BAKE_TIER_STATIC);
        assert.equal(posts.length, 1);
        assert.equal(posts[0].payload.centerX, 0);
        const near = scheduler.enqueue("bakeGroundChunk", chunkPayload({ chunkKey: packChunkKey(9, 0), centerX: 200, centerY: 0 }), TILE_BAKE_TIER_STATIC);
    ...
    it("stats reflect queue and busy workers", () => {
        ...
        scheduler.enqueue("bakeGroundChunk", chunkPayload(), TILE_BAKE_TIER_STATIC);
