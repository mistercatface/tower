...
async function createEditorTestState() {
    ...
    const cavernConfig = { boundsMode: "rect", boundsCol: 0, boundsRow: 0, boundsCols: 32, boundsRows: 32 };
    ...
    const state = {
        ...
        editor: { showSelectionRings: true, cavernConfig, railConfig: { ...cavernConfig }, railMazeConfig: { ...cavernConfig } },
        nav: navigation,
    };
...
describe("boid explore behavior", () => {
    ...
    it("re-picks a target when stuck, and uses fallback if no nav walkable cells are found", async () => {
        ...
        for (let idx = 0; idx < grid.grid.length; idx++) grid.grid[idx] = 1; // Blocked
