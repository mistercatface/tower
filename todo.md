...
- [ ] **`button_bumper` 3D asset** — pushable/impact-activated button; same `buttonLinks` schema; `activation: { kind: "impact", minImpulse }`; normal 3D draw + push collision (not `spatialRole: "trigger"`)
- [ ] **`poweredLinkId` on strategy** — optional declarative powered target on gravity props (button links cover pinball today)
